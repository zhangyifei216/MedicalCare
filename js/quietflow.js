		...
		if (theme == "squareFlash"){
			...
			for (var i = 0; i < bounceBallCount; i++){
				...
				if (specificColors.length == 0){
					if (transparent){
						circleData.push([Math.random() * $limitX, Math.random() * $limitY, Math.random() * maxRadius, Math.random() * 2, Math.random() * 4, randCol(255, 255, 255, .3)]);
					} else {
			...
			for (var i = 0; i < triangles; i++){
				...
				if (specificColors.length > 0){
					...
					ctx.fillStyle = randCol(255, 255, 255, .3);
				}
			...
			for (var i = 0; i < boxCount; i++){
				...
				if (specificColors.length == 0){
					if (transparent){
						squareData.push([Math.random() * $limitX*2 - $limitX, Math.random() * $limitY, Math.random() * maxBoxSize + 1, randCol(255, 255, 255, .3), Math.random() * 5]);
					} else {
			...
			function floatingBoxesTimed(){
				...
				for (var i = 0; i < squareData.length; i++){
					...
					if (current[X] > $limitX + maxBoxSize || current[Y] < -maxBoxSize){
						...
						if (specificColors.length == 0){
							if (transparent){
								squareData.push([Math.random() * $limitX*2 - $limitX, Math.random() * $limitY*2 + $limitY, Math.random() * maxBoxSize + 1, randCol(255, 255, 255, .3), Math.random() * 5]);
							} else {
